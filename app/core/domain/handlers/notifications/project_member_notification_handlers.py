...
async def notification_on_member_removed(ev: ProjectMemberRemovedEvent):
    async with async_session_maker() as session:
        message = (
            f"You have been removed from the project '{ev.project_title}' by "
            f"{ev.performed_name}."
        )
        ...
        notifications = await write_notifications(
            ...
            actor_id=ev.performed_by,  # type: ignore
            message=message,
            notif_type=EventType.PROJECT_MEMBER_REMOVED,
            project_id=ev.project_id,
            session=session,
        )
...
def register_event_handlers():
    ...
    subscribe_background(ProjectMemberUpdatedEvent, notification_on_member_updated)
    subscribe_background(ProjectMemberRemovedEvent, notification_on_member_removed)
